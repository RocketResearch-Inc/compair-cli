...
func workspacePath() (string, error) {
	path, err := appdir.Path("workspace.db")
	if err != nil {
		return "", err
	}
...
func (s *Store) ListUnderPrefix(ctx context.Context, prefix, group string) ([]TrackedItem, error) {
	...
	for rows.Next() {
		var ti TrackedItem
		if err := rows.Scan(&ti.ID, &ti.Path, &ti.Kind, &ti.GroupID, &ti.DocumentID, &ti.RepoRoot, &ti.LastSyncedCommit, &ti.FileSig, &ti.ContentHash, &ti.Size, &ti.MTime, &ti.LastSyncedAt, &ti.LastSeenAt, &ti.Published); err != nil {
			return nil, err
		}
		out = append(out, ti)
	}
...
func (s *Store) FindByPathGroup(ctx context.Context, path, group string) (*TrackedItem, error) {
	...
	var ti TrackedItem
	if err := row.Scan(&ti.ID, &ti.Path, &ti.Kind, &ti.GroupID, &ti.DocumentID, &ti.RepoRoot, &ti.LastSyncedCommit, &ti.FileSig, &ti.ContentHash, &ti.Size, &ti.MTime, &ti.LastSyncedAt, &ti.LastSeenAt, &ti.Published); err != nil {
		return nil, err
	}
	return &ti, nil
}
