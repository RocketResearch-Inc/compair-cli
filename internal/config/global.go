...
func GlobalConfigPath() (string, error) {
    d, err := globalDir(); if err != nil { return "", err }
    return filepath.Join(d, "config.yaml"), nil
}

func ReadGlobal() (Global, error) {
    p, err := GlobalConfigPath(); if err != nil { return Global{}, err }
    b, err := os.ReadFile(p)
    if err != nil { if os.IsNotExist(err) { return Global{}, nil }; return Global{}, err }
    var g Global
    if err := yaml.Unmarshal(b, &g); err != nil { return Global{}, err }
    return g, nil
}

func WriteGlobal(g Global) error {
    p, err := GlobalConfigPath(); if err != nil { return err }
    b, _ := yaml.Marshal(g)
...
func ResolveActiveGroup(flagVal string) (string, error) {
    if v := os.Getenv("COMPAIR_ACTIVE_GROUP"); strings.TrimSpace(v) != "" { return v, nil }
    if strings.TrimSpace(flagVal) != "" { return flagVal, nil }
    d, err := globalDir(); if err != nil { return "", err }
    p := filepath.Join(d, "active_group")
    b, err := os.ReadFile(p)
    if err == nil { v := strings.TrimSpace(string(b)); if v != "" { return v, nil } }
