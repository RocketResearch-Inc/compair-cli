...
func errorGuidance(cmd *cobra.Command, message string) []string {
	...
	if containsAny(msg,
		...
		add("If you do want to keep waiting locally, rerun the same command or use 'compair wait --timeout 20m' to continue waiting (or 'compair wait --timeout 0' to wait indefinitely).")
		if localCore {
